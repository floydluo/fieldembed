...
class BasicObject(object):
    ...
    def INIT(cls, CORPUSPath, 
        ...
        CORPUS['EndIDXGroups'] = np.array(CORPUS['EndIDXGroups'], dtype = np.uint64)
        CORPUS['length']       = len(CORPUS['EndIDXGroups'])

        GROUP['EndIDXTexts']   = np.array(GROUP['EndIDXTexts'],  dtype = np.uint64)
        GROUP['length']        = len(GROUP['EndIDXTexts'])

        TEXT['EndIDXSents']    = np.array(TEXT['EndIDXSents'],   dtype = np.uint64)
        TEXT['length']         = len(TEXT['EndIDXSents'])

        SENT['EndIDXTokens']   = np.array(SENT['EndIDXTokens'],  dtype = np.uint64)
        SENT['length']         = len(SENT['EndIDXTokens'])
        for ch, path in cls.Channel_Hyper_Path.items():
            SENT[path] = np.array(SENT[path],  dtype = np.uint64)
